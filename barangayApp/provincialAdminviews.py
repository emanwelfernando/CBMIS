...
def audit_log_view(request):
    ...
    return render(request, 'provincialAdmin/audit_log.html', {'page_obj': page_obj})

@login_required
@permission_required('barangayApp.can_view_provincial_admin_views', raise_exception=True)
def dashboard(request):
    ...
    return render(request, 'provincialAdmin/dashboard.html', context)
...
def household(request, page=1):
    ...
    return render(request, 'provincialAdmin/household.html', context)
...
def resident(request, page=1):
    ...
    return render(request, 'provincialAdmin/resident.html', context)
...
def add_municipal(request):
    if request.method=="POST":
        if request.POST.get('name') \
            ...
            return HttpResponseRedirect(reverse('provincial-admin-municipal'))

    else:
            return render(request, 'provincialAdmin/add_municipal.html')
...
def edit_municipal(request, pk):
    ...
    if request.method == "POST":
        ...
        return HttpResponseRedirect('/provincial-admin/municipal')
    else:
        return render(request, 'provincialAdmin/edit_municipal.html')
...
def add_barangay(request):
    if request.method == "POST":
        ...
        return HttpResponseRedirect('/provincial-admin/barangay')
    else:
        form = BarangayForm()
    ...
    return render(request, 'provincialAdmin/add_barangay.html', context)
...
def edit_user(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('provincial-admin-manage-user')
    else:
    ...
    return render(request, 'provincialAdmin/edit_user.html', context)
...
def delete_barangay(request, pk):
    ...
    barangay.delete()
    return redirect("provincial-admin-barangay")

def register(request):
    if request.method == 'POST':
        form = ProvincialUserRegistrationForm(request.POST)
        if form.is_valid():
            user = form.save()  
            return redirect('provincial-admin-manage-user')  
    else:
        form = ProvincialUserRegistrationForm() 
    
    context = {'form': form}
    return render(request, 'provincialAdmin/register_user.html', context)
...
def manage_users(request, page=1):
    ...
    user_types_to_filter = ['municipal_admin']
    ...
    users = CustomUser.objects.filter(user_type_condition)
    ...
    for user in page_obj:
        form = ProvincialUserRegistrationForm(instance=user)
    ...
    context = {
        ...
        'page_obj': page_obj,
        'form': ProvincialUserRegistrationForm()  # Use the same form class for registration and editing
    }
    
    return render(request, 'provincialAdmin/manage_users.html', context)
...
def change_password(request):
    ...
    return render(request, 'provincialAdmin/change_password.html', {'form': form})

@login_required
@permission_required('barangayApp.can_view_provincial_admin_views', raise_exception=True)   
def user_settings(request):
    return render(request, 'provincialAdmin/user_settings.html')